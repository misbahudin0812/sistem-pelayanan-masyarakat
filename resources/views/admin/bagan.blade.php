<x-admin-layout>
    <h1 class="text-2xl font-bold">Bagan Desa</h1>
    @if (session('status'))
    ...
    @endif
    <a href="{{ route('tambah.bagan') }}" class="btn btn-sm btn-success my-3"><span class="fa fa-plus"></span>Tambah
        Data</a>
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @foreach ($staff as $data)
                            <tr
                                ...
                                <td class="px-4 py-3 text-sm">{{$data->nip}}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <div class="join lg:join-horizontal">
                                        <a href="{{ route('edit.bagan', ['id' => $data->id]) }}"
                            ...
                            <div class="modal" role="dialog" id="deleteConfirmation{{ $data->id }}">
                                <div class="modal-box">
                                    ...
                                    <p class="py-2">Apakah anda yakin akan mengapus data {{ $data->nama }}?</p>
                                    <div class="modal-action">
                                        <form method="dialog">
                                            <!-- if there is a button in form, it will close the modal -->
                                            <a href="#" class="btn">Tidak</a>
                                        </form>
                                        <form action="{{ route('hapus.bagan', ['id' => $data->id]) }}" method="post">
                                            @csrf
                                            @method('delete')
                                            <x-danger-button>
                                                {{ __('Hapus Data') }}
